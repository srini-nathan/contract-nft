...
contract ExchangeNFT is ERC721Holder, Ownable, Pausable, PullPayment {
    ...
    event Unbid(address indexed bidder, uint256 indexed tokenId, uint256 amount);
    ...
    function createListing(uint256 _tokenId, uint256 _price) public whenNotPaused {
        require(!listings.contains(_tokenId), "listing already exists");
        require(msg.sender == nft.ownerOf(_tokenId), "you are not the owner of this token");
    ...
    function changeListingPrice(uint256 _tokenId, uint256 _price) public listingExists(_tokenId) whenNotPaused {
        require(tokenSellers[_tokenId] == msg.sender, "only seller can change listing price");
        require(_price > 0, "Price must be granter than zero");
    ...
    function removeListing(uint256 _tokenId) public listingExists(_tokenId) whenNotPaused {
        require(tokenSellers[_tokenId] == msg.sender, "only seller can remove listing");
    ...
    function buyToken(uint256 _tokenId) public payable listingExists(_tokenId) whenNotPaused {
        uint256 price = listings.get(_tokenId);
        address seller = tokenSellers[_tokenId];
        require(msg.value > 0, "ETH value zero");
        require(msg.value > price, "ETH value should be greater");
    ...
    function bidOnToken(uint256 _tokenId, uint256 _amount) public whenNotPaused {
        require(listings.contains(_tokenId), "listing doesnt exist");
        require(userBids[msg.sender][_tokenId] == 0, "you already have a bid, cancel it first");
        uint256 length = tokenBids[_tokenId].length;
        require(_amount > tokenBids[_tokenId][length - 1], "bid should be greater than previous bidder");
    ...
    function cancelBid(uint256 _tokenId) public whenNotPaused {
        require(listings.contains(_tokenId), "listing doesnt exist");
        require(userBids[msg.sender][_tokenId] != 0, "you dont have a bid on this listing");
    ...
    function sellViaBidding(uint256 _tokenId) public listingExists(_tokenId) whenNotPaused {
        require(tokenSellers[_tokenId] == msg.sender, "only owner of token can call");

        uint256 length = tokenBids[_tokenId].length;
        uint256 price = tokenBids[_tokenId][length - 1];
        address buyer = indexToAddress[_tokenId][length - 1];
        address seller = tokenSellers[_tokenId];
        //require(nft._transfer(buyer, seller, price), "TransferFrom failed");
    ...
    function getBidByUser(uint256 _tokenId, address addr) public view returns (uint256) {
        return userBids[addr][_tokenId];
    }

    function getAllBids(uint256 _tokenId) public view returns (uint256[] memory) {
